<template>
	<view class="container">
		...
		<view class="main">
			...
			<view class="content">
				<input placeholder="请输入商品名称" maxlength="300" class="content-input"
					v-model="imgInfo.name"></input>
			</view>
			<view class="content">
				<input type="number" placeholder="请输入商品价格" maxlength="100" class="content-input"
					v-model="imgInfo.price"></input>
			</view>
			<view class="content">
				<input type="number" placeholder="请输入商品原价" maxlength="100" class="content-input"
					v-model="imgInfo.old"></input>
			</view>
			<view class="content">
				<input type="number" placeholder="请输入商品库存" maxlength="100" class="content-input"
					v-model="imgInfo.num"></input>
			</view>
			<view class="content">
				<textarea placeholder="请输入商品介绍" maxlength="300" class="content-input"
	...
	export default {
		...
		methods: {
			...
			chooseImg() {
				...
				// let maxCount = 1 - that.imgList.length
				uni.chooseImage({
			...
			next() {
				...
				if(this.imgInfo.length == 0){
					...
					uni.getFileSystemManager().readFile({
					    ...
					    encoding: 'base64',
					   success: (res) => {
							this.imgInfo.image = 'data:image/jpeg;base64,' + res.data;
					    },
					    fail: (errr) => {
					    }
					})
				...
				setTimeout(() => {
						sendGoods(this.imgInfo).then(res=>{
							if(res.success){
								uni.hideLoading()
								uni.showToast({
									title: "发布商品成功"
