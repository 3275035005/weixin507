	...
	<view class="container">
		...
		<view class="fotter">
			<view class="add">
				...
				<tui-button type="danger" shape="circle" @click="publish" limit='1' :disabled="disabled">点击发布
				</tui-button>
			</view>
		</view>
		...
		methods: {
			...
			getUserInfo(uid, type) {
				getUserInfoById(uid).then(res => {
					...
					if (type == 1) {
						this.value = res.data.row.avatar
					} else {
						this.value = res.data.row.image
					}
